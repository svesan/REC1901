...
proc sort data=gnu.npr_relatives; by relatives_id diag_dat index;run;
proc sort data=gnu.npr_parents;   by parents_id diag_dat index;run;
proc sort data=gnu.npr_child;     by child_id diag_dat index;run;
...
*----------------------------------------------------------------;
proc sql;
  create index diag on gnu.npr_relatives(diag);
  create index diag on gnu.npr_parents(diag);
  create index diag on gnu.npr_children(diag);

  create index icddiag on gnu.npr_relatives(icd,diag);
  create index icddiag on gnu.npr_parents(icd,diag);
  create index icddiag on gnu.npr_children(icd,diag);
quit;

endrsubmit;

proc contents data=gnu.npr_relatives;run;
